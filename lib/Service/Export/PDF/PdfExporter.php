...
class PdfExporter {
    ...
    private function createPdfDocument() {
        // Wenn TCPDF verfügbar ist, echtes Dokument nutzen
        if (class_exists('TCPDF')) {
            $pdf = new \TCPDF('P', 'mm', 'A4', true, 'UTF-8', false);
            // Basis-Dokumenteinstellungen für saubere Ausgabe
            $pdf->SetCreator('Nextcloud Verein');
            $pdf->SetAuthor('Nextcloud Verein');
            $pdf->SetTitle('Vereinsdokument');
            $pdf->SetSubject('Export');
            $pdf->SetKeywords('PDF, Verein, Export');
            // Unicode-fähige Schrift
            // DejaVu Sans ist in TCPDF enthalten und unterstützt Umlaute/Unicode
        ...
        // Fallback-Mock für Entwicklungsumgebungen ohne TCPDF
        return new class {
            ...
            public function setPrintFooter($v) {}
            public function MultiCell($w, $h, $txt, $border = 0, $align = 'L', $fill = false, $ln = 1) {}
            public function getAliasNumPage() { return '1'; }
            public function getAliasNbPages() { return '1'; }
    ...
    private function addHeader($pdf, array $clubData): void {
        ...
        // Vereinsname und Daten (Unicode-tauglich falls TCPDF)
        $pdf->SetFont(class_exists('TCPDF') ? 'dejavusans' : 'helvetica', 'B', 14);
        $pdf->SetXY(50, 15);
        $pdf->Cell(0, 5, $clubData['name']);
        
        $pdf->SetFont(class_exists('TCPDF') ? 'dejavusans' : 'helvetica', '', 9);
        $pdf->SetXY(50, 22);
    ...
    private function addAddressBlock($pdf, array $member): void {
        $pdf->SetFont(class_exists('TCPDF') ? 'dejavusans' : 'helvetica', '', 10);
        $pdf->SetXY(15, 45);
    ...
    private function addInvoiceDetails($pdf, array $clubData, array $transaction): void {
        $pdf->SetFont(class_exists('TCPDF') ? 'dejavusans' : 'helvetica', 'B', 11);
        $pdf->SetXY(130, 45);
    ...
    private function addItemsTable($pdf, array $transaction): void {
        $pdf->SetFont(class_exists('TCPDF') ? 'dejavusans' : 'helvetica', 'B', 10);
        $pdf->SetXY(15, 75);
    ...
    private function addSummary($pdf, array $transaction): void {
        ...
        $pdf->SetFont(class_exists('TCPDF') ? 'dejavusans' : 'helvetica', 'B', 12);
        $pdf->Cell(50, 6, '€ ' . number_format($transaction['amount'], 2, ',', '.'), 0, 1, 'R');
    }
    
    private function addFooter($pdf, array $clubData): void {
        // Einheitliche Fußzeile mit Seitenzahlen
        if (class_exists('TCPDF') && $pdf instanceof \TCPDF) {
            // TCPDF: eigene Footer-Ausgabe pro Seite
            $footerText = ($clubData['name'] ?? '') . ' | ' . ($clubData['email'] ?? '') . ' | ' . date('d.m.Y H:i');
            // Position von unten
            $pdf->SetY(-15);
            $pdf->SetFont('dejavusans', '', 8);
            $pdf->Cell(0, 5, $footerText, 0, 0, 'L');
            $pdf->Cell(0, 5, 'Seite ' . $pdf->getAliasNumPage() . '/' . $pdf->getAliasNbPages(), 0, 0, 'R');
            return;
        }
